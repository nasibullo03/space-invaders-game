...
bool fire = false, weaponFire = true, leftFireRotation = false, rightFireRotation = false, clickOnKeyboardUP = false, first = true, 
     hitTheTarget = false, shotOnProtection = false, dropingBomb = true, newGame = true, gameOver = true, goingUpLetter = true;
...
void settingsImages() {
    ...
    int weaponXP, weaponYP;
    weaponXP = WSize[0].width / 2 - 75;
    weaponYP = WSize[0].height - 170;

    weaponSprite.setPosition(float(weaponXP), float(weaponYP));//задаем началные позиции оружя
...
void fontsProsession() {
    ...
    for (int textIndex = 0; textIndex < 13; ++textIndex) {
        newGameText[textIndex].setFont(newGameFont);
        newGameText[textIndex].setCharacterSize(60);
        newGameText[textIndex].setStyle(Text::Bold);
    }
    ...
    for (int textIndex = 0; textIndex < 5; ++textIndex) {
        newGameText[textIndex].setPosition(startCoordinationX, startCoordinationY);
    ...
    for (int textIndex = 6; textIndex < 13; ++textIndex) {
        newGameText[textIndex].setPosition(startCoordinationX, startCoordinationY);
        startCoordinationX += 32;
    }
...
void startANewGame(float time) {
    ...
    for (int textIndex = 0; textIndex < 13; ++textIndex) {
        ...
        window.draw(newGameText[textIndex]);
    }
